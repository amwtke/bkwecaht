...
namespace BK.DataSyncFromEK
{
    public class UserRepository: IDisposable
    {
        ...
        {
            this.context = new EKDBContext();
        }
        public UserRepository(EKDBContext context)
        {
        ...
        //    }
        //}
        public async Task<List<T>> GetUserRecords<T>(Guid input) where T : class, IDBModelWithID
        {
            List<T> uclist = null;
            if(input != Guid.Empty)
                uclist = await (from uc in context.Set<T>()
                                where uc.AccountEmail_uuid == input
