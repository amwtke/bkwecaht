...
namespace BK.CommonLib.DB.Repositorys
{
    public class UserRepository: IDisposable
    {
        ...
        #region 8项资料通用方法
        public async Task<T> GetUserRecordsById<T>(T input) where T : DBModelBase, IDBModelWithID
        {
            T result;
        ...
        public async Task<bool> SaveUserRecordsById<T>(T input) where T : DBModelBase, IDBModelWithID
        {
            if(input.Id == 0)
            {
        ...
        public async Task<bool> DeleteUserRecordsById<T>(T input) where T : DBModelBase, IDBModelWithID
        {
            T get = await (from up in context.Set<T>()
                           where up.Id == input.Id
                           select up).FirstOrDefaultAsync();
        ...
        public async Task<List<T>> GetUserRecords<T>(T input) where T : DBModelBase, IDBModelWithID
        {
            List<T> uclist = null;
        ...
        public async Task<List<T>> GetUserRecordsByUuid<T>(Guid uuid) where T : DBModelBase, IDBModelWithID
        {
            List<T> uclist = null;
        ...
        public List<T> GetUserRecords_TB<T>(T input) where T : DBModelBase, IDBModelWithID
        {
            List<T> uclist = null;
            if (input.AccountEmail_uuid != Guid.Empty)
        ...
        public async Task<Tuple<int, int, List<T>>> GetUserRecords<T>(T input, int pageIndex, int pageSize) where T : DBModelBase, IDBModelWithID
        {
            List<T> uclist = null;
            int itemCount = 0;
            int PageCount = 0;
        ...
        public async Task<int> GetUserRecordsNumber<T>(T input) where T : DBModelBase, IDBModelWithID
        {
            int result = 0;
