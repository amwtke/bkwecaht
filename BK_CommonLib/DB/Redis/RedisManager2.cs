...
namespace BK.CommonLib.DB.Redis
{
    ...
    {
        static ConcurrentDictionary<Type, ConnectionMultiplexer> _dic = new ConcurrentDictionary<Type, ConnectionMultiplexer>();
        static object syncobject2 = new object();

        object syncobject = new object();
        ConnectionMultiplexer _con = null;
        ...
        public IDatabase GetDb<RedisObjectType>()
        {
            ...
            {
                LogHelper.LogErrorAsync(typeof(RedisManager2<T>), ex);
                return null;
            }
        }
        ...
        {
            T config = BK.Configuration.BK_ConfigurationManager.GetConfig<T>();
            ...
            op.ClientName = config.GetType().ToString() + "-ip:" + BK.CommonLib.Util.CommonHelper.GetLocalIp() + "-" + DateTime.Now.ToString();
        ...
        public async Task<bool> SaveObjectAsync(Object o)
        {
            ...
            if(numberAtt == null)
            {
                LogHelper.LogErrorAsync(typeof(RedisManager2<T>), new Exception("Redis的存储对象没有指定dbNumber属性！"));
        ...
        /// <returns></returns>
        public async Task<RedisObjectType> GetObjectFromRedis<RedisObjectType>(string key) where RedisObjectType : new()
        {
            try
            {
        ...
        public RedisObjectType GetObjectFromRedis_TongBu<RedisObjectType>(string key) where RedisObjectType : new()
        {
            try
            {
                string hashName = GetHashName<RedisObjectType>();
        ...
        string getKeyAttributeValue(object o)
        {
            foreach(var p in o.GetType().GetProperties())
            {
                ...
                if(att != null)
                {
                    ...
                    if(value == null)
                    {
                        LogHelper.LogErrorAsync(typeof(RedisManager2<T>), new Exception("type=" + o.GetType() + "在saveByAttAsync时，没有指定Key！"));
        ...
        #region 删除hash项
        public async Task<bool> DeleteHashItemAsync<RedisObject,HashEntryAtt>(string hashKeyName,string key) where HashEntryAtt : RedisBaseAttribute
        {
            string entryName = GetKeyName<RedisObject, HashEntryAtt>();
            key = ObjectHelper.MakeField(key, entryName);
            var db = GetDb<RedisObject>();
            return await db.HashDeleteAsync(hashKeyName, key);
        }

        public bool DeleteHashItem<RedisObject, HashEntryAtt>(string hashKeyName,string key) where HashEntryAtt : RedisBaseAttribute
        {
            string entryName = GetKeyName<RedisObject, HashEntryAtt>();
            key = ObjectHelper.MakeField(key, entryName);
            var db = GetDb<RedisObject>();
            return db.HashDelete(hashKeyName, key);
        }
