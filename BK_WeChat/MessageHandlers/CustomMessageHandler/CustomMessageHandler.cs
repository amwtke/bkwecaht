...
using BK_WeChat.CommonService.Utilities;
...
namespace BK_WeChat.CommonService.CustomMessageHandler
{
    /// <summary>
    /// 自定义MessageHandler
    ...
    public partial class CustomMessageHandler : MessageHandler<CustomMessageContext>
    {
        ...
        static BK.Model.Configuration.WeixinConfig wexinConfig = BK_ConfigurationManager.GetConfig<BK.Model.Configuration.WeixinConfig>();
#else
        ...
        static BK.Model.Configuration.WeixinConfig wexinConfig = BK_ConfigurationManager.GetConfig<BK.Model.Configuration.WeixinConfig>();
        private string agentToken = wexinConfig.WeixinToken;
        //WebConfigurationManager.AppSettings["WeixinAgentToken"];//Token
...
测试完成后，您可以登陆公众号后台取消授权。",
            ...
            else
            {
                ...
                try
                {
                    km = Convert.ToDouble(requestMessage.Content);

                    //LogHelper.LogInfoAsync(typeof(CustomMessageHandler), "KM=" + km.ToString());

                    if (km > 0)
                    {
                        var list = ComplexLocationManager.GetDistanceInKmById(requestMessage.FromUserName, km);

                        //LogHelper.LogInfoAsync(typeof(CustomMessageHandler), "List count=" + list.Count.ToString());
                        ...
                        if (list.Count > 0)
                        {
                            ...
                            {
                                var userInfo = WXAuthHelper.GetUserInfoByOPenId_TongBu(l.Id);

                                //LogHelper.LogInfoAsync(typeof(CustomMessageHandler), "nice name=" + userInfo.NiceName);

                                if (userInfo != null && !string.IsNullOrEmpty(userInfo.NiceName))
                                {
                                    double distance = ComplexLocationManager.GetDistanceBetween(requestMessage.FromUserName, l.Id);

                                    //LogHelper.LogInfoAsync(typeof(CustomMessageHandler), "Distence=" + distance.ToString());


                                    sb.AppendLine(userInfo.NiceName + "->" + distance + "KM!");
                                }
                            }
                        ...
                        {
                            responseMessage.Content += "没有找到KM=" + km.ToString() + "内的人！";
                        }
                    }
                    else
                    {
                        responseMessage.Content += "KM=" + km.ToString() + "不能小于0！";
                    }
                }
                catch (Exception ex)
                {
                    requestMessage.Content += "KM=" + km.ToString();
                ...
                if (km == 0)
                {
                    ...
                    result.AppendLine("您的OPENID or uniionID为:" + requestMessage.FromUserName);
                    responseMessage.Content = result.ToString();

                    //BK.CommonLib.Log.LogHelper.LogInfoAsync(typeof(CustomMessageHandler), " OPID:" + RequestMessage.FromUserName+" 发送了："+ result.ToString());
                }
            }
        ...
        {
            bool updateOK = ComplexLocationManager.AddOrUpdateLocation(requestMessage.FromUserName, requestMessage.Location_X, requestMessage.Location_Y);
